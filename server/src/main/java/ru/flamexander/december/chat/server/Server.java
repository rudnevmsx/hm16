...
public class Server {
    ...
    public synchronized void kickUser(String adminUsername, String kickedUsername, ClientHandler clientHandler) {
        if (userService.isUserAdmin(adminUsername)){
            if (clientHandler.getUsername() == kickedUsername){
                clients.remove(clientHandler);
                broadcastMessage("Администратор кикнул пользователя " + kickedUsername);
            } else {
                System.out.println("Указан несуществующий пользователь");
            }
        }
